...
explore: date_table {
  ...
  join: batch_report_summary {
    type: left_outer
    sql_on: ${practice2.gx_provider_id} = ${batch_report_summary.gx_provider_id}  and ${batch_report_summary.sales_created_at_date} = ${date_table.date_date};;
    relationship: one_to_many
  }
  join: customer2 {
    type: left_outer
    sql_on: ${customer2.gx_customer_id} = ${batch_report_summary.gx_customer_id};;
    relationship: many_to_one
  }
  ...
  sql_always_where: ${batch_report_summary.sales_type} IS NOT NULL and ${practice2.gx_provider_id} is not NULL and ${batch_report_summary.category} = 'GOOD';;
}
...
explore: batch_report_details {
  ...
  join: customer2 {
    type: left_outer
    sql_on: ${customer2.gx_customer_id} = ${batch_report_details.gx_customer_id};;
    relationship: many_to_one
  }

  join: practice2 {
    type: left_outer
    sql_on: ${practice2.gx_provider_id} = ${batch_report_details.gx_provider_id};;
    relationship: many_to_one
  }
