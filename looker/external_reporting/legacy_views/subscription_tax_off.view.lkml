view: subscription_tax_off {
  derived_table: {
    sql: select prov.name as provider_name,
      ...
      s.*
      from gaia.subscription s
      left join gaia.plan p on s.plan_id = p.id
      left join gaia.customer c on p.customer_id = c.id
      left join gaia.provider prov on c.provider_id = prov.id
