view: today_upcoming_deposits_derived_2 {
  derived_table: {
  ...
  measure: upcoming_deposit {
    type: sum
    #sql: coalesce((coalesce(coalesce(${sum_tran_to_date},0)-coalesce(${sum_fund_to_date},0),0))/100,0);;
  ...
  dimension: total_fund {
    type: number
    sql: ${TABLE}.total_fund ;;
  }

  dimension: total_tran {
    type: number
    sql: ${TABLE}.total_tran ;;
  }
