view: product_sales {
  ...
  dimension: top_five_products {
    ...
    html:  <p style="text-align:center;color:black;font-size:140%"> Top Product Sales </p>
      <div  style="text-align:center; number-format='$#,##0';color:white;font-size:10% ">{{ rendered_value }}</div> ;;
  }

  dimension: data_by_device {
    type: string
    sql: case when ${subscription_created_date} IS NOT NULL then 'Total by Device' else NULL end ;;
    html:  <p style="text-align:center;color:black;font-size:155%"> Total by Device </p>
      <div  style="text-align:center; number-format='$#,##0';color:white;font-size:10% ">{{ rendered_value }}</div> ;;
  }

  dimension: data_by_card_brand {
    type: string
    sql: case when ${subscription_created_date} IS NOT NULL then 'Total by Card Brand' else NULL end ;;
    html:  <p style="text-align:center;color:black;font-size:155%"> Total by Card Brand </p>
      <div  style="text-align:center; number-format='$#,##0';color:white;font-size:10% ">{{ rendered_value }}</div> ;;
  }

  dimension: Blank_tile {
    type: string
    sql: case when ${subscription_created_date} IS NOT NULL then ' ' else NULL end ;;
    html:  <p style="text-align:center;color:black;font-size:155%">   </p>
      <div  style="text-align:center; number-format='$#,##0';color:white;font-size:10% ">{{ rendered_value }}</div> ;;
  }
