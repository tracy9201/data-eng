  ...
  derived_table:{
    sql:  select payment_summary.*, subscription_name, user_type
              ...
              left join (
                  ...
                  from (
                      select subscription_name, dim_customer.gx_customer_id as gx_cus_id, user_type,
                      ...
                      join dwh_opul.dim_customer dim_customer on product_sales.k_customer_id = dim_customer.gx_customer_id
                      where user_type=1
                  ) sub_cus
                  where sub_created = 1
            ) pro_cus on gx_customer_id = gx_cus_id
    ;;
  }
...
## ------------------ HIDDEN HELPER DIMENSIONS  ------------------ ##
  ...
  dimension: days_from_start_first {
    ...
    hidden: yes
    type: number
    sql: DATEDIFF('day',  {% date_end first_end_period_filter %},{% date_start first_start_period_filter %}) ;;
  }
  ...
  dimension: days_from_start_second {
    ...
    hidden: yes
    type: number
    sql: DATEDIFF('day',  {% date_end second_end_period_filter %}, {% date_start second_start_period_filter %}) ;;
  }
...
## ------------------ DIMENSIONS TO PLOT ------------------ ##
  ...
  dimension: period_selected {
    ...
    sql:
        CASE
            WHEN ${sales_created_at_date} BETWEEN timestamp with time zone 'epoch' + {{ _user_attributes['start_date'] }} * interval '1 second'
              AND  timestamp with time zone 'epoch' + {{ _user_attributes['end_date'] }} * interval '1 second'
            THEN 'First Period'
            WHEN ${sales_created_at_date} BETWEEN  timestamp with time zone 'epoch' + {{ _user_attributes['comparison_start_date'] }} * interval '1 second'
              AND  timestamp with time zone 'epoch' + {{ _user_attributes['comparison_end_date'] }} * interval '1 second'
            THEN 'Second Period'
            END ;;
  }
...
## Filtered measures
  ...
  measure: current_period_sales {
    view_label: "_PoP"
    type: sum
    sql: ${sales_amount_tipping};;
    value_format: "$#,##0.00"
    filters: {
      field: period_selected
      value: "First Period"
    }
  }
  ...
  measure: sum_total {
    type: sum
    sql: coalesce(${sales_amount},0)+coalesce(${gratuity_amount},0) ;;
    label: " Total"
    value_format: "$#,##0.00"
  }
  #######################################
  ...
  dimension: sales_amount_tipping {
    type: number
    sql: CASE WHEN ${transaction} = 'REFUND' then coalesce((-1*${TABLE}.sales_amount)/100,0) + round(cast(${TABLE}.gratuity_amount as numeric)/100,2)
          WHEN ${transaction} = 'VOID' then coalesce((-1*${TABLE}.sales_amount)/100,0) + round(cast(${TABLE}.gratuity_amount as numeric)/100,2)
          ELSE coalesce(${TABLE}.sales_amount/100,0) + round(cast(${TABLE}.gratuity_amount as numeric)/100,2) end ;;
    label: "Amount"
    value_format: "$#,##0.00"
  }
  ...
  dimension: payment_id {
    type: string
    sql: case
              when ${sales_type} = 'CREDIT CARD' or ${payment_method} in('OTHER CREDIT CARD','VISA','AMEX','MASTERCARD','DISCOVER') then CONCAT('**** ',cast(${TABLE}.payment_id as VARCHAR))
  ...
  dimension: description {
    type: string
    sql: case
          when ${TABLE}.user_type =1 then ${TABLE}.subscription_name
          when ${sales_type} in ('CHECK', 'CREDIT CARD', 'BD') then null
  ...
  dimension: category {
    type: string
    sql: case when (${sales_id} like 'void1%' or ${sales_id} like 'void2%') and ${is_voided} = 'Yes' then 'BAD'
              when ${TABLE}.sales_type = 'adjustment' then 'BAD'
              else 'GOOD' end ;;
