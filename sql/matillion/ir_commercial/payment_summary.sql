...
(   SELECT distinct
    'payment_'||p.id::varchar  AS sales_id, 
     ...
     ,card.brand
FROM
    internal_gaia_hint.payment p
LEFT JOIN
    internal_gaia_hint.subscription sub
        ON sub.id = p.subscription_id
LEFT JOIN
    internal_gaia_hint.gratuity gratuity
        ON gratuity.id = p.gratuity_id
LEFT JOIN
(SELECT distinct payment_id,card_payment_gateway_id
  FROM  internal_gaia_hint.gateway_transaction
  WHERE card_payment_gateway_id IS NOT NULL) gt
        ON gt.payment_id = p.id
LEFT JOIN
    internal_gaia_hint.card_payment_gateway cpg
        ON cpg.id = gt.card_payment_gateway_id
LEFT JOIN
    internal_gaia_hint.card card
...
WHERE
 ...
 (    SELECT
     ...
     ,card.brand
 FROM
     internal_gaia_hint.refund refund
 LEFT JOIN
     internal_gaia_hint.gratuity gratuity
 ...
 LEFT JOIN
     internal_gaia_hint.invoice_item ivi
