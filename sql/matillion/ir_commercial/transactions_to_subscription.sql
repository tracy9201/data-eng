...
,t.customer_type, pmt.id as pmt_id,  gt.invoice_item_id
...
left join gaia.gateway_transaction gt on gt.source_object_id = pmt.id and gt.source_object_name = 'payment'
...
, t.customer_type, cr.id as cr_id,  gt.invoice_item_id
, ks.type as subscription_type
, ks.offering_id
, ks.ad_hoc_offering_id
, od.id as od_id
...
left join gaia.gateway_transaction gt on gt.source_object_id = cr.id and gt.source_object_name = 'credit'
left join gaia.invoice_item item on item.id = gt.invoice_item_id
left join gaia.subscription gs on gs.id = item.subscription_id
left join kronos.subscription ks on ks.gx_subscription_id  = gs.encrypted_ref_id
left join kronos.users u on u.id = t.user_id
