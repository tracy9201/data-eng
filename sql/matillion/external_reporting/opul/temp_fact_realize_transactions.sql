WITH ful_sale as (
  SELECT 
    ...
    round(cast(sub.subtotal as numeric)/100,2) as subtotal,
    ...
    kcus.gx_customer_id,
    org.gx_provider_id
  FROM gaia_opul.fulfillment ful
  left join 
  ...
  left join 
    gaia_opul.customer cus 
      on cus.id = plan.customer_id
  left join 
    kronos_opul.customer_data kcus
      on kcus.gx_customer_id = cus.encrypted_ref_id
  left join 
    kronos_opul.users kuser
...
offering as (
  select 
    ...
    null as offering_id
  from gaia_opul.invoice_item inv
  left join 
    gaia_opul.subscription sub
      on sub.id = subscription_id
  where inv.status = 20
...
main as (
  select 
    ...
    ful_sale.quantity,
    ful_sale.subtotal,
    ful_refund.refund_amount,
    ful_sale.subtotal/ful_sale.quantity *ful_sale.quantity_rendered - coalesce(refund_amount,0) as final_amount,
