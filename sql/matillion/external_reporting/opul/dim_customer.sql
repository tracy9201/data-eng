...
WITH sub_plan AS (SELECT
   ...
   FROM
       kronos_opul.subscription subscription
   WHERE
       subscription.status=0
   GROUP BY
       plan_id ),  main AS ( SELECT
       customer_data.id AS k_customer_id,
       to_timestamp(member_on_boarding_date,
       'YYYY-MM-DD HH24:MI:SS') as member_on_boarding_date,
       to_timestamp(member_cancel_date,
       'YYYY-MM-DD HH24:MI:SS') as member_cancel_date,
