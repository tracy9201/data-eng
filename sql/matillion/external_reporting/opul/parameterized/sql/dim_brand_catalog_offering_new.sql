...
select  
    ...
    round(cast(master_catalog_item.wholesale_price as numeric )/100,2) as master_wholesale_price, 
    master_catalog_item.sku,
    round(cast(practice_catalog_item.wholesale_price as numeric )/100,2) as practice_wholesale_price,
  ...
  WHERE practice_catalog_item.id is not null
)
SELECT * FROM main