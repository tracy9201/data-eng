  ...
  FROM (
        SELECT 
            ...
            CASE 
              ...
              END AS isMember2,
            gx_customer_id
        FROM kronos_opul.customer_data customer
        JOIN 
        ...
        -- We don't need TO have subscription id, if there ARE several subscription WITH the same created AND deprecated date, we just care that AT that TIME we had a subscription
            SELECT 
                ...
                CASE 
                    ...
                    (CASE WHEN (deprecated_at IS NOT null) THEN (cast(to_char(deprecated_at,'MON-DD-YYYY') AS date)) 
                    ...
                    ELSE (cast(to_char(deprecated_at,'MON-DD-YYYY') AS date)) END AS deprecated_date,
                    -- Member or not
                    CASE WHEN (type=2 OR type=1) THEN 1 ELSE 0 END AS isMember
            FROM kronos_opul.subscription subscription
