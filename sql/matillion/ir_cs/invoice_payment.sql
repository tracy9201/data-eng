WITH Invoice_level as (
  ...
  FROM
    ...
    left join (
      select
        invoice_id,
        count(*) as count_of_invoice_item
      from
        internal_gaia_hint.invoice_item
      group by
        1
    ) invoice_item on invoice_id = invoice.id
...
Offering as (
  SELECT
    ...
    sku
  FROM
    internal_kronos_hint.subscription subscription
  ...
  WHERE
    subscription.status in (0, 7)
),
Invoice_Item_level as (
  select
    'invoice_' || cast(invoice.id AS text) AS id1,
    'invoice_' || cast(invoice.id AS text)|| '_sub_' || cast(
      coalesce(
        subscription.id, invoice_item.id
      ) AS text
    ) AS id2,
    case when invoice.pay_date is null then invoice.updated_at else invoice.pay_date end as pay_date,
    invoice.id as invoice,
    ...
    (
      cast(
        subscription.subtotal as NUMERIC(18, 2)
      )+ cast(
        invoice_item.discounts as NUMERIC(18, 2)
      )
    )/ cast(
      subscription.quantity as NUMERIC(18, 2)
    )* 100 as price_unit,
    (
      subscription.subtotal + invoice_item.discounts
    ) as total_price,
...
main as (
  SELECT
    ...
    round(
      cast(
        (
          case when id like '%sub%' then total_price else (
            sum(total_price) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ) as total_price,
    case when id like '%sub%' then recurring_payment else (
      sum(recurring_payment) over (partition by invoice)
    )/(
      count(recurring_payment) over (partition by invoice)
    ) end as recurring_payment,
    round(
      cast(
        (
          case when id like '%sub%' then invoice_amount else (
            sum(invoice_amount) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ...
    round(
      cast(
        (
          case when id like '%sub%' then discounted_Price else (
            sum(discounted_Price) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ...
    round(
      cast(
        (
          case when id like '%sub%' then tax_charged else (
            sum(tax_charged) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ...
    round(
      cast(
        (
          case when id like '%sub%' then taxable_amount else (
            sum(taxable_amount) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ...
    round(
      cast(
        (
          case when id like '%sub%' then item_discount else (
            sum(item_discount) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ) as item_discount,
    case when id like '%sub%' then discount_reason else (
      case when (
        sum(
          case when discount_reason is not null then 1 else 0 end
        ) over (partition by invoice)
      )> 0 then 'Various' else null end
    ) end as discount_reason,
    round(
      cast(
        (
          case when id like '%sub%' then grand_total else (
            sum(grand_total) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ) as grand_total,
    union_table.gx_subscription_id,
    gx_customer_id,
    gx_provider_id,
    count_of_invoice_item,
    round(
      cast(
        (
          case when id like '%sub%' then invoice_actual_amount else (
            sum(invoice_actual_amount) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ) as invoice_actual_amount,
    round(
      cast(
        (
          case when id like '%sub%' then invoice_credit else (
            sum(invoice_credit) over (partition by invoice)
          ) end
        ) as numeric
      )/ 100,
      2
    ...
    case when max(count_brand) over(partition by invoice) = 1
    and id not like '%sub%' then first_value(brand ignore nulls) over (
      partition by invoice
      order by
        count_brand rows between unbounded preceding
        and unbounded following
    ) when max(count_brand) over(partition by invoice) > 1
    and id not like '%sub%' then 'Various' else brand end as brand,
    max(count_product) over(partition by invoice) as count_distinct_product,
    case when max(count_product) over(partition by invoice) = 1
    and id not like '%sub%' then first_value(product_service ignore nulls) over (
      partition by invoice
      order by
        count_brand rows between unbounded preceding
        and unbounded following
    ) when max(count_product) over(partition by invoice) > 1
    and id not like '%sub%' then 'Various' else product_service end as product_service,
    max(count_sku) over(partition by invoice) as count_distinct_sku,
    sku
  FROM
    union_table
    LEFT JOIN Offering o on o.gx_subscription_id = union_table.gx_subscription_id
  order by
    invoice,
    id
)
