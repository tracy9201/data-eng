...
SELECT
    ...
    FROM
        ...
        LEFT JOIN
            v4_k_plan k_plan 
                ON plan.encrypted_ref_id = k_plan.gx_plan_id     
        LEFT JOIN
            v4_k_customer_data  customer 
                ON k_plan.user_id = customer.user_id     
        LEFT JOIN
            v4_k_organization_data provider 
                ON customer.organization_id = provider.id   
        WHERE
        ...
        LEFT JOIN
            v4_k_organization_data provider 
                ON customer.organization_id = provider.id     
        LEFT JOIN
            v4_g_gateway_transaction gateway_transaction 
        ...
        LEFT JOIN
            v4_g_subscription subscription 
        ...
        LEFT JOIN
            v4_g_invoice_item invoice_item 
        ...
        LEFT JOIN
            v4_g_subscription 
                ON subscription_id = v4_g_subscription.id     
        LEFT JOIN
            v4_g_gateway_transaction gateway_transaction 
        ...
        SELECT
            'settle_'||cast(settlement.id AS text) AS sales_id,
            ...
            NULL as reason   
        FROM
            v4_g_authorisation authorisation     
        RIGHT JOIN
            v4_g_settlement settlement 
                ON authorisation.id = settlement.authorisation_id     
        LEFT JOIN
        ...
        LEFT JOIN
            v4_k_plan plan2 
                on plan.encrypted_ref_id = plan2.gx_plan_id     
        LEFT JOIN
            v4_k_customer_data customer 
                on plan2.user_id = customer.user_id     
        LEFT JOIN
            v4_k_organization_data provider 
                ON customer.organization_id = provider.id   
        WHERE
        ...
        LEFT JOIN
            v4_g_payment payment 
                ON  payment.id = gateway_transaction.payment_id     
        LEFT JOIN
            v4_g_plan plan 
                ON payment.plan_id = plan.id     
        LEFT JOIN
